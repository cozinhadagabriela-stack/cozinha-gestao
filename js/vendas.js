...
function getCondicaoPagamentoLabel(valor) {
  return String(valor || "").trim() === "prazo" ? "A prazo" : "À vista";
}

function sincronizarDataPagamentoVenda() {
  if (!salePaymentDateInput || !saleDateInput || !salePaymentConditionSelect) return;
  const dataVenda = (saleDateInput.value || "").trim();
  const condicao = (salePaymentConditionSelect.value || "vista").trim();

  if (condicao === "vista") {
    salePaymentDateInput.value = dataVenda || salePaymentDateInput.value || toISODateLocal(new Date());
  } else if (!salePaymentDateInput.value) {
    salePaymentDateInput.value = dataVenda || toISODateLocal(new Date());
  }
}
...
if (saleDateInput) saleDateInput.addEventListener("change", sincronizarDataPagamentoVenda);
if (salePaymentConditionSelect) salePaymentConditionSelect.addEventListener("change", sincronizarDataPagamentoVenda);
sincronizarDataPagamentoVenda();
    ...
    while (true) {
      ...
      snapshot.forEach((doc) => {
        ...
        if ((!v.dataTimestamp || !isFinite(Number(v.dataTimestamp))) && v.data) {
          v.dataTimestamp = (typeof erpDateInputToTimestamp === "function" ? erpDateInputToTimestamp(v.data) : new Date(v.data).getTime());
        }
...
// Barras - Top 5 clientes por faturamento (adapta para horizontal no mobile)
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: function (context) {
              const valor = Number(mobile ? context.parsed?.x : context.parsed?.y || 0);
              const base = Number(totalValor || 0);
              const perc = base > 0 ? (valor / base) * 100 : 0;
              return `${formatarMoedaBR(valor)} (${formatarPercent(perc)})`;
            }
          }
        }
      },
...
async function excluirVenda(vendaId) {
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      const lote = String(venda.lote || "").trim();
      const produtoId = venda.produtoId || "";
      const quantidade = Number(venda.quantidade || 0);

      if (lote && produtoId && quantidade > 0) {
        const estoqueRef = db.collection("estoque").doc(`${produtoId}__${lote || "SEMLOTE"}`);
        const estoqueSnap = await tx.get(estoqueRef);
        const estoqueAtual = estoqueSnap.exists ? (estoqueSnap.data() || {}) : {};
        const qtdAtual = Number(estoqueAtual.quantidade || 0);
        ...
        tx.set(
          ...
          {
            produtoId,
            produtoDescricao: venda.produtoDescricao || estoqueAtual.produtoDescricao || "",
            lote,
            dataValidade: estoqueAtual.dataValidade || "",
            codigoBarras: estoqueAtual.codigoBarras || "",
            quantidade: qtdNova,
    ...
    if (typeof carregarClientesAReceber === "function") {
      try {
        await carregarClientesAReceber();
      } catch (e) {
        console.error("Erro ao atualizar clientes a receber após excluir venda:", e);
      }
    }
    ...
    if (typeof carregarEstoqueSaldos === "function") {
      try {
        ...
        console.error("Erro ao atualizar estoque após excluir venda:", e);
      }
    }
  } catch (e) {
...
exportCsvButton.addEventListener("click", () => {
  ...
  filtradas.forEach((v) => {
    const linha = [
      ...
      csvValue(v.condicaoPagamentoLabel || getCondicaoPagamentoLabel(v.condicaoPagamento)),
      csvValue(formatarDataBrasil(v.dataPagamento || "")),
      csvValue(v.statusRecebimento || ""),
...
saveSaleButton.addEventListener("click", async () => {
  ...
  const condicaoPagamento = (salePaymentConditionSelect?.value || "vista").trim() === "prazo" ? "prazo" : "vista";
  const dataPagamento = (salePaymentDateInput?.value || "").trim();
  ...
  if (!dataPagamento) {
    saleMessage.textContent = "Informe a data de pagamento.";
    saleMessage.className = "msg error";
    return;
  }

  if (condicaoPagamento === "prazo" && dataPagamento < dataStr) {
    saleMessage.textContent = "Para vendas a prazo, a data de pagamento não pode ser anterior à data da venda.";
    saleMessage.className = "msg error";
    return;
  }
  ...
  try {
    const dataTimestamp =
      typeof erpDateInputToTimestamp === "function"
        ? erpDateInputToTimestamp(dataStr)
        : new Date(dataStr).getTime();
    const dataPagamentoTimestamp =
      typeof erpDateInputToTimestamp === "function"
        ? erpDateInputToTimestamp(dataPagamento)
        : new Date(dataPagamento).getTime();

    if (!Number.isFinite(dataTimestamp)) {
      throw new Error("Data da venda inválida.");
    }
    if (!Number.isFinite(dataPagamentoTimestamp)) {
      throw new Error("Data de pagamento inválida.");
    }

    const itensNormalizados = itensParaSalvar.map((item) => {
      ...
      if (!item.produtoId) {
        throw new Error("Há item do pedido sem produto selecionado.");
      }
      if (!Number.isFinite(quantidade) || quantidade <= 0) {
        throw new Error(`Quantidade inválida para ${produto.descricao || item.produtoDescricao || "o item"}.`);
      }
      if (!Number.isFinite(valorUnitario) || valorUnitario <= 0) {
        throw new Error(`Valor unitário inválido para ${produto.descricao || item.produtoDescricao || "o item"}.`);
      }

      return {
        produtoId: item.produtoId,
        produtoDescricao: produto.descricao || item.produtoDescricao || "",
        quantidade,
        valorUnitario,
    ...
    itensNormalizados.forEach((item) => {
      if (!item.lote) return;
      const chave = `${item.produtoId}__${item.lote}`;
      const atual = estoqueMap.get(chave) || {
        produtoId: item.produtoId,
        lote: item.lote,
        produtoDescricao: item.produtoDescricao || "",
        quantidadeSaida: 0,
        ref: db.collection("estoque").doc(chave),
      };
      atual.quantidadeSaida += item.quantidade;
      if (!atual.produtoDescricao) {
        atual.produtoDescricao = item.produtoDescricao || "";
      }
      estoqueMap.set(chave, atual);
    });

    const vendaRefs = itensNormalizados.map(() => db.collection("vendas").doc());

    const condicaoPagamentoLabel = getCondicaoPagamentoLabel(condicaoPagamento);
    const statusRecebimento = condicaoPagamento === "prazo" ? "pendente" : "recebido";

    await db.runTransaction(async (tx) => {
      const saldosAtuais = new Map();

      for (const [chave, info] of estoqueMap.entries()) {
        const snap = await tx.get(info.ref);
        const dadosAtuais = snap.exists ? (snap.data() || {}) : {};
        const saldoAtual = Number(dadosAtuais.quantidade || 0);

        if (!snap.exists) {
          throw new Error(`Lote ${info.lote} de ${info.produtoDescricao || "produto"} não encontrado no estoque.`);
        }

        if (saldoAtual < info.quantidadeSaida) {
          throw new Error(
            `Estoque insuficiente para ${info.produtoDescricao || "produto"} (lote ${info.lote}). Saldo atual: ${saldoAtual}.`
          );
        }

        saldosAtuais.set(chave, dadosAtuais);
      }

      itensNormalizados.forEach((item, idx) => {
        tx.set(vendaRefs[idx], {
          usuarioId: user.uid,
          data: dataStr,
          dataTimestamp,
          clienteId,
          ...
          dataPagamento,
          dataPagamentoTimestamp,
          statusRecebimento,
          recebidoEm: statusRecebimento === "recebido"
      ...
      for (const [chave, info] of estoqueMap.entries()) {
        const dadosAtuais = saldosAtuais.get(chave) || {};
        const saldoAtual = Number(dadosAtuais.quantidade || 0);
        const saldoNovo = saldoAtual - info.quantidadeSaida;
        ...
        if (saldoNovo === 0) {
          ...
          tx.set(
            info.ref,
            {
              ...
              produtoDescricao: info.produtoDescricao || dadosAtuais.produtoDescricao || "",
              lote: info.lote,
              dataValidade: dadosAtuais.dataValidade || "",
              codigoBarras: dadosAtuais.codigoBarras || "",
    ...
    saleDateInput.value = hojeIso;
    saleClientSelect.value = "";
    salePaymentSelect.value = "";
    if (salePaymentConditionSelect) salePaymentConditionSelect.value = "vista";
    if (salePaymentDateInput) salePaymentDateInput.value = hojeIso;
    ...
    if (typeof carregarClientesAReceber === "function") {
      try {
        ...
        console.error("Erro ao atualizar clientes a receber após venda:", e);
      }
    }
    ...
    saleMessage.textContent = e?.message || "Erro ao salvar venda.";
    saleMessage.className = "msg error";
  }
});
