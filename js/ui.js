...
const sectionTitles = {
  venda: "Lançamento de vendas",
  "lancamento-despesas": "Lançamento de despesas",
  extrato: "Extrato / Relatório",
  estoque: "Estoque",
  cadastros: "Cadastros",
  caixa: "Financeiro",
  ajuda: "Ajuda ?"
...
function updateTopbarTitle(section) {
  if (!viewTitle) return;
  viewTitle.textContent = sectionTitles[section] || "Painel principal";
}

function updateTodayLabel() {
  if (!todayLabel) return;
  const agora = new Date();
  todayLabel.textContent = agora.toLocaleDateString("pt-BR", {
    weekday: "long",
    day: "2-digit",
    month: "long",
    year: "numeric"
...
const subtabExtratoCustos = document.getElementById("subtab-extrato-custos");
...
const extratoCustosWrap = document.getElementById("extrato-custos-wrap");
...
const sectionAjuda = document.getElementById("section-ajuda");
const subtabCaixaCaixa = document.getElementById("subtab-caixa-caixa");
const subtabCaixaReceber = document.getElementById("subtab-caixa-receber");
const caixaWrap = document.getElementById("caixa-wrap");
const caixaReceberWrap = document.getElementById("caixa-receber-wrap");
...
const salePaymentConditionSelect = document.getElementById("sale-payment-condition");
const salePaymentDateInput = document.getElementById("sale-payment-date");
...
function setActiveSection(section) {
  ...
  if (sectionAjuda) sectionAjuda.classList.add("hidden");
  ...
  if (tabAjuda) tabAjuda.classList.remove("active-tab");
  ...
  if (section === "venda") {
    ...
    // Ao entrar em Financeiro, carrega/atualiza os dados e abre Caixa por padrão
    if (typeof initCaixaUI === "function") {
      initCaixaUI();
    }
    if (typeof setCaixaSubtab === "function") {
      setCaixaSubtab("caixa");
    }
  } else if (section === "ajuda") {
    if (sectionAjuda) sectionAjuda.classList.remove("hidden");
    if (tabAjuda) tabAjuda.classList.add("active-tab");
  }
}
...
function setExtratoSubtab(subtab) {
  ...
  if (extratoCustosWrap) extratoCustosWrap.classList.add("hidden");
  ...
  if (subtabExtratoCustos) subtabExtratoCustos.classList.remove("active-tab");
  ...
  } else if (subtab === "custos") {
    if (extratoCustosWrap) extratoCustosWrap.classList.remove("hidden");
    if (subtabExtratoCustos) subtabExtratoCustos.classList.add("active-tab");

    if (typeof initCustosProdutos === "function") {
      initCustosProdutos();
    }
...
function setCaixaSubtab(subtab) {
  if (!caixaWrap || !caixaReceberWrap || !subtabCaixaCaixa || !subtabCaixaReceber) return;

  caixaWrap.classList.add("hidden");
  caixaReceberWrap.classList.add("hidden");

  subtabCaixaCaixa.classList.remove("active-tab");
  subtabCaixaReceber.classList.remove("active-tab");

  if (subtab === "receber") {
    caixaReceberWrap.classList.remove("hidden");
    subtabCaixaReceber.classList.add("active-tab");
  } else {
    caixaWrap.classList.remove("hidden");
    subtabCaixaCaixa.classList.add("active-tab");
  }
}
...
function updateUI(user) {
  if (user) {
    ...
    if (salePaymentConditionSelect && !salePaymentConditionSelect.value) {
      salePaymentConditionSelect.value = "vista";
    }
    if (salePaymentDateInput && !salePaymentDateInput.value) {
      salePaymentDateInput.value = (saleDateInput && saleDateInput.value)
        ? saleDateInput.value
...
if (subtabExtratoCustos) subtabExtratoCustos.addEventListener("click", () => setExtratoSubtab("custos"));
...
if (tabAjuda) tabAjuda.addEventListener("click", () => setActiveSection("ajuda"));
if (subtabCaixaCaixa) subtabCaixaCaixa.addEventListener("click", () => setCaixaSubtab("caixa"));
if (subtabCaixaReceber) subtabCaixaReceber.addEventListener("click", () => setCaixaSubtab("receber"));
