...
async function carregarProdutos() {
  try {
    ...
    // atualiza o total (R$) do estoque de produtos acabados (depende do preço cadastrado)
    if (typeof atualizarTotalEstoqueProdutosAcabados === "function") {
...
async function existeReferenciaNaColecao(collectionName, fieldName, value) {
  if (!db || !collectionName || !fieldName || !value) return false;
  const snap = await db
...
async function excluirCliente(id) {
  ...
  try {
    const possuiVendas = await existeReferenciaNaColecao("vendas", "clienteId", id);
    if (possuiVendas) {
      alert("Este cliente já possui vendas vinculadas e não pode ser excluído.");
      return;
    }
...
async function excluirProduto(id) {
  ...
  try {
    const [possuiVendas, possuiEstoque] = await Promise.all([
      existeReferenciaNaColecao("vendas", "produtoId", id),
      existeReferenciaNaColecao("estoque", "produtoId", id),
    ]);

    if (possuiVendas || possuiEstoque) {
      alert("Este produto já possui vendas ou saldo em estoque e não pode ser excluído.");
      return;
    }
...
async function excluirForma(id) {
  ...
  try {
    const [possuiVendas, possuiDespesas] = await Promise.all([
      existeReferenciaNaColecao("vendas", "formaId", id),
      existeReferenciaNaColecao("despesas", "formaId", id),
    ]);

    if (possuiVendas || possuiDespesas) {
      alert("Esta forma de pagamento já está em uso e não pode ser excluída.");
      return;
    }
    ...
    alert(e?.message || "Erro ao excluir forma de pagamento.");
  }
}
