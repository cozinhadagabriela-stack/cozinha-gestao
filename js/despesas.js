...
async function catPropagarAtualizacao(oldNome, newNome, newGrupo) {
  ...
  // Itens
  const itensSnap = await db.collection("itens").where("categoria", "==", oldN).get();
  for (const doc of itensSnap.docs) {
    const upd = {};
    if (newN && oldN !== newN) upd.categoria = newN;
    if (newG) upd.categoriaGrupo = newG;
    if (Object.keys(upd).length) {
      await db.collection("itens").doc(doc.id).update(upd);
    }
  }
...
function renderizarCategoriasDespesas() {
  ...
  ordenada.forEach((c) => {
    ...
    btnExcluir.addEventListener("click", async () => {
      ...
      try {
        const nome = c.nome || "";
        const itensSnap = await db.collection("itens").where("categoria", "==", nome).limit(1).get();
...
async function excluirFornecedor(id) {
  ...
  try {
    const despesasRef = await db.collection("despesas").where("fornecedorId", "==", id).limit(1).get();
    if (!despesasRef.empty) {
      alert("Este fornecedor já possui despesas vinculadas e não pode ser excluído.");
      return;
    }
    ...
    alert(e?.message || "Erro ao excluir fornecedor.");
  }
}
...
function iniciarEdicaoItemDespesa(itemId) {
  ...
  itemDespesaEditandoId = itemId;  if (itemDescricaoInput) itemDescricaoInput.value = item.descricao || "";
...
function cancelarEdicaoItemDespesa() {
  itemDespesaEditandoId = null;  if (itemDescricaoInput) itemDescricaoInput.value = "";
...
async function carregarItensDespesas() {
  ...
  try {
    ...
    if (itensDespesaTbody) {
      itensDespesaTbody.innerHTML =
        '<tr><td colspan="6">Erro ao carregar itens.</td></tr>';
    }
  }
}
...
async function salvarItemDespesa() {
  ...
  if (!descricao) {
    if (itemMessage) {
      itemMessage.textContent = "Informe a descrição do item.";
  ...
  try {
    ...
    if (itemDespesaEditandoId) {
      await db.collection("itens").doc(itemDespesaEditandoId).update(payload);
      if (itemMessage) {
...
async function excluirItemDespesa(id) {
  ...
  try {
    const [despesasRef, estoqueRef] = await Promise.all([
      db.collection("despesas").where("itemDespesaId", "==", id).limit(1).get(),
      db.collection("estoqueInsumos").where("itemId", "==", id).limit(1).get(),
    ]);

    if (!despesasRef.empty || !estoqueRef.empty) {
      alert("Este item já está em uso em despesas ou no estoque de insumos e não pode ser excluído.");
      return;
    }
    ...
    alert(e?.message || "Erro ao excluir item de despesa.");
  }
}
...
function atualizarSelectItensFornecedor(_fornecedorId) {
  if (!despItemSelect) return;

  despItemSelect.innerHTML =
    '<option value="">Selecione um item (opcional)</option>';

  const lista = Array.isArray(itensDespesasCache) ? itensDespesasCache : [];
  if (lista.length === 0) {
...
function preencherFormasPagamentoDespesas() {
  ...
  const entries = Object.entries(formasMap)
    .map(([id, data]) => ({ id, descricao: (data?.descricao || "").trim() }))
    .sort((a, b) => a.descricao.localeCompare(b.descricao));

  if (!entries.length) {
    despFormaSelect.innerHTML = '<option value="">Nenhuma forma cadastrada</option>';
    return;
  }

  despFormaSelect.innerHTML = '<option value="">Selecione a forma de pagamento</option>';

  entries.forEach(({ id, descricao }) => {
    const opt = document.createElement("option");
    opt.value = id;
    opt.textContent = descricao || "(sem descrição)";
...
async function carregarTodasDespesasBrutas() {
  const out = [];
  ...
  while (true) {
    ...
    snap.forEach((doc) => {
      const d = doc.data() || {};
      let ts = Number(d.dataPagamentoTimestamp || 0);
      if (!Number.isFinite(ts) || ts <= 0) {
        ts = typeof erpDateInputToTimestamp === "function"
          ? erpDateInputToTimestamp(d.dataPagamento)
          : new Date(d.dataPagamento).getTime();
      }

      out.push({
        id: doc.id,
        ...d,
        dataPagamentoTimestamp: Number.isFinite(ts) && ts > 0 ? ts : null,
      });
    });
...
function calcularCustoMedioReversaoEstoqueInsumo(estAtual, qtdReversao, custoUnitReversao) {
  const qtdAtual = Number(estAtual?.quantidade || 0);
  const custoAtual = Number(estAtual?.custoMedio || 0);
  const qtd = Number(qtdReversao || 0);
  const custo = Number(custoUnitReversao || 0);

  const qtdNova = qtdAtual - qtd;
  if (qtdNova < 0) {
    throw new Error("Não é possível excluir a despesa porque parte desse estoque já foi consumida.");
  }
  if (qtdNova === 0) {
    return { qtdNova: 0, custoNovo: 0 };
  }

  const valorAtual = qtdAtual * custoAtual;
  const valorReversao = qtd * custo;
  const valorNovo = valorAtual - valorReversao;

  if (valorNovo < -0.000001) {
    throw new Error("Não foi possível recalcular o custo médio desse insumo com segurança.");
  }

  const custoNovo = valorNovo <= 0 ? 0 : (valorNovo / qtdNova);
  return {
    qtdNova,
    custoNovo: Number.isFinite(custoNovo) ? custoNovo : 0,
  };
}


function despesaObterTimestampEntradaEstoque(dados) {
  const tsEntrada = Number(dados?.dataEntradaEstoqueTimestamp);
  if (Number.isFinite(tsEntrada) && tsEntrada > 0) return tsEntrada;

  const dataEntrada = String(dados?.dataEntradaEstoque || "").trim();
  if (typeof erpDateInputToTimestamp === "function") {
    const ts = erpDateInputToTimestamp(dataEntrada);
    if (Number.isFinite(ts) && ts > 0) return ts;
  }

  const tsPagamento = Number(dados?.dataPagamentoTimestamp);
  if (Number.isFinite(tsPagamento) && tsPagamento > 0) return tsPagamento;

  const dataPagamento = String(dados?.dataPagamento || "").trim();
  if (typeof erpDateInputToTimestamp === "function") {
    const ts = erpDateInputToTimestamp(dataPagamento);
    if (Number.isFinite(ts) && ts > 0) return ts;
  }
...
function despesaExtrairUltimaEntradaRestante(despesasDocs, despesaExcluidaId) {
  let melhor = null;

  (despesasDocs || []).forEach((docSnap) => {
    if (!docSnap || docSnap.id === despesaExcluidaId) return;
    const d = typeof docSnap.data === "function" ? (docSnap.data() || {}) : (docSnap || {});
    if (d.gerouEstoqueInsumo !== true) return;

    const ts = despesaObterTimestampEntradaEstoque(d);
    if (!Number.isFinite(ts) || ts <= 0) return;

    const custoUnit = Number(d.estoqueInsumoCustoUnitEntrada ?? d.valorUnitario ?? 0);
    const dataIso = typeof erpTimestampToIsoDate === "function"
      ? erpTimestampToIsoDate(ts)
      : String(d.dataEntradaEstoque || d.dataPagamento || "").trim();

    if (!melhor || ts > melhor.timestamp) {
      melhor = {
        timestamp: ts,
        dataIso,
        custoUnit: Number.isFinite(custoUnit) ? custoUnit : null,
      };
    }
  });
...
async function carregarDespesas() {
  ...
  try {
    despesasCache = await carregarTodasDespesasBrutas();

    if (!despesasCache.length) {
      despesasTbody.innerHTML =
...
function aplicarFiltrosDespesas() {
  ...
  if (start) {
    filtradas = filtradas.filter((d) => {
      const dataIso = String(d.dataPagamento || "").slice(0, 10);
      return dataIso && dataIso >= start;
    });
  }
...
function despAplicarFiltrosEmMemoriaPeriodo(startIso, endIso) {
  ...
  if (startIso) {
    filtradas = filtradas.filter((d) => {
      const dataIso = String(d.dataPagamento || "").slice(0, 10);
      return dataIso && dataIso >= startIso;
    });
  }
    ...
    if (labelsForn.length && total > 0) {
      ...
      chartDespFornecedores = new Chart(ctxForn, {
        ...
        options: {
          ...
          maintainAspectRatio: false,
          layout: {
            padding: { top: mobile ? 8 : 0, right: 6, bottom: 4, left: 4 }
          },
          plugins: {
            legend: { display: false },
            tooltip: {
              callbacks: {
                label: function (context) {
                  const v = Number(mobile ? context.parsed?.x : context.parsed?.y || 0);
                  const perc = total > 0 ? (v / total) * 100 : 0;
...
async function salvarDespesa() {
  ...
  try {
    const dataTimestamp = typeof erpDateInputToTimestamp === "function" ? erpDateInputToTimestamp(dataPag) : new Date(dataPag).getTime();
    const dataEntradaTimestamp = dataEntrada ? (typeof erpDateInputToTimestamp === "function" ? erpDateInputToTimestamp(dataEntrada) : new Date(dataEntrada).getTime()) : null;
    ...
    await db.runTransaction(async (tx) => {
      ...
      let estAtual = null;
      let estoquePrevUltimaEntradaData = "";
      let estoquePrevUltimaEntradaDataTimestamp = null;
      let estoquePrevUltimoCustoUnit = null;
      if (deveGerarEstoque && estoqueRef) {
        const estSnap = await tx.get(estoqueRef);
        estAtual = estSnap.exists ? (estSnap.data() || {}) : {};
        estoquePrevUltimaEntradaData = String(estAtual?.ultimaEntradaData || "").trim();
        estoquePrevUltimaEntradaDataTimestamp = Number.isFinite(Number(estAtual?.ultimaEntradaDataTimestamp))
          ? Number(estAtual.ultimaEntradaDataTimestamp)
          : null;
        estoquePrevUltimoCustoUnit = Number.isFinite(Number(estAtual?.ultimoCustoUnit))
          ? Number(estAtual.ultimoCustoUnit)
          : null;
      }
      ...
      tx.set(despRef, {
        ...
        estoqueInsumoPrevUltimaEntradaData: deveGerarEstoque ? estoquePrevUltimaEntradaData : null,
        estoqueInsumoPrevUltimaEntradaDataTimestamp: deveGerarEstoque ? estoquePrevUltimaEntradaDataTimestamp : null,
        estoqueInsumoPrevUltimoCustoUnit: deveGerarEstoque ? estoquePrevUltimoCustoUnit : null,
...
async function excluirDespesa(id) {
  ...
  try {
    ...
    if (dados && dados.gerouEstoqueInsumo === true && dados.estoqueInsumoDocId) {
      ...
      const qtdReversao = Number(dados.estoqueInsumoDeltaQtd || 0);
      const custoReversao = Number(dados.estoqueInsumoCustoUnitEntrada || 0);
      const despesasMesmoEstoqueQuery = db
        .collection("despesas")
        .where("estoqueInsumoDocId", "==", dados.estoqueInsumoDocId);
      const despesasMesmoEstoqueSnap = await despesasMesmoEstoqueQuery.get();
      const ultimaEntradaRestante = despesaExtrairUltimaEntradaRestante(
        despesasMesmoEstoqueSnap?.docs || [],
        id
      );
      const ultimaEntradaSnapshotAnterior = {
        dataIso: String(dados.estoqueInsumoPrevUltimaEntradaData || "").trim(),
        timestamp: Number.isFinite(Number(dados.estoqueInsumoPrevUltimaEntradaDataTimestamp))
          ? Number(dados.estoqueInsumoPrevUltimaEntradaDataTimestamp)
          : null,
        custoUnit: Number.isFinite(Number(dados.estoqueInsumoPrevUltimoCustoUnit))
          ? Number(dados.estoqueInsumoPrevUltimoCustoUnit)
          : null,
      };
      const ultimaEntradaParaRestaurar = ultimaEntradaRestante || (
        ultimaEntradaSnapshotAnterior.dataIso || Number.isFinite(Number(ultimaEntradaSnapshotAnterior.timestamp))
          ? ultimaEntradaSnapshotAnterior
          : null
      );
      ...
      await db.runTransaction(async (tx) => {
        ...
        const { qtdNova, custoNovo } = calcularCustoMedioReversaoEstoqueInsumo(
          estAtual,
          qtdReversao,
          custoReversao
        );
        ...
        if (qtdNova === 0) {
          ...
          const payloadEstoque = {
            quantidade: qtdNova,
            custoMedio: custoNovo,
            atualizadoEm: firebase.firestore.FieldValue.serverTimestamp(),
          };

          if (ultimaEntradaParaRestaurar) {
            payloadEstoque.ultimaEntradaData = ultimaEntradaParaRestaurar.dataIso || "";
            payloadEstoque.ultimaEntradaDataTimestamp = Number.isFinite(Number(ultimaEntradaParaRestaurar.timestamp))
              ? Number(ultimaEntradaParaRestaurar.timestamp)
              : null;
            payloadEstoque.ultimaEntradaEm = null;
            if (Number.isFinite(Number(ultimaEntradaParaRestaurar.custoUnit))) {
              payloadEstoque.ultimoCustoUnit = Number(ultimaEntradaParaRestaurar.custoUnit);
            } else {
              payloadEstoque.ultimoCustoUnit = null;
            }
          } else {
            payloadEstoque.ultimaEntradaData = "";
            payloadEstoque.ultimaEntradaDataTimestamp = null;
            payloadEstoque.ultimaEntradaEm = null;
            payloadEstoque.ultimoCustoUnit = null;
          }

          tx.set(estoqueRef, payloadEstoque, { merge: true });
        }
    ...
    alert(e?.message || "Erro ao excluir despesa.");
  }
}
...
async function salvarLancamentoDespesas() {
  ...
  try {
    const itensValidos = itensLancamentoDespesa.map((it) => {
      ...
      if (!descricao || quantidade <= 0 || valorUnit <= 0 || valorTotal <= 0) {
        throw new Error("Há item de despesa com dados inválidos.");
      }
    ...
    itensValidos.forEach((it) => {
      if (!it.deveGerarEstoque || !it.itemInfo || !it.estoqueInsumoDocId) return;
      const atual = ajustesEstoque.get(it.estoqueInsumoDocId) || {
        ref: db.collection("estoqueInsumos").doc(it.estoqueInsumoDocId),
        itemInfo: it.itemInfo,
        itemGrupo: it.itemGrupo,
        quantidadeTotal: 0,
        valorTotalEntrada: 0,
      };
      atual.quantidadeTotal += it.quantidade;
      atual.valorTotalEntrada += (it.quantidade * it.valorUnit);
      ajustesEstoque.set(it.estoqueInsumoDocId, atual);
    });

    const despRefs = itensValidos.map(() => db.collection("despesas").doc());

    await db.runTransaction(async (tx) => {
      const estoqueAtualMap = new Map();
      const estoquePrevMap = new Map();

      for (const [docId, info] of ajustesEstoque.entries()) {
        const snap = await tx.get(info.ref);
        const estAtual = snap.exists ? (snap.data() || {}) : {};
        estoqueAtualMap.set(docId, estAtual);
        estoquePrevMap.set(docId, {
          ultimaEntradaData: String(estAtual?.ultimaEntradaData || "").trim(),
          ultimaEntradaDataTimestamp: Number.isFinite(Number(estAtual?.ultimaEntradaDataTimestamp))
            ? Number(estAtual.ultimaEntradaDataTimestamp)
            : null,
          ultimoCustoUnit: Number.isFinite(Number(estAtual?.ultimoCustoUnit))
            ? Number(estAtual.ultimoCustoUnit)
            : null,
        });
      }
      ...
      for (const [docId, info] of ajustesEstoque.entries()) {
        const estAtual = estoqueAtualMap.get(docId) || {};
        const qtdAtual = Number(estAtual.quantidade || 0);
        const custoAtual = Number(estAtual.custoMedio || 0);
        const qtdNova = qtdAtual + info.quantidadeTotal;
        const custoNovo = qtdNova > 0
          ? (((qtdAtual * custoAtual) + info.valorTotalEntrada) / qtdNova)
          : 0;

        tx.set(info.ref, {
          itemId: info.itemInfo.id,
          itemDescricao: info.itemInfo.descricao || "",
          unidade: info.itemInfo.unidade || "",
          categoria: info.itemInfo.categoria || "",
          grupo: info.itemGrupo || "",
          lote: "",
          dataValidade: "",
          codigoBarras: info.itemInfo.codigoBarras || "",
          quantidade: qtdNova,
          ...
          ultimaEntradaData: dataEntrada,
          ultimaEntradaDataTimestamp: Number.isFinite(dataEntradaTimestamp) ? dataEntradaTimestamp : null,
          ultimoCustoUnit: info.quantidadeTotal > 0 ? (info.valorTotalEntrada / info.quantidadeTotal) : null,
        }, { merge: true });
      }

      itensValidos.forEach((it, idx) => {
        tx.set(despRefs[idx], {
          usuarioId: user.uid,
          fornecedorId,
          fornecedorNome,
          ...
          dataEntradaEstoque: dataEntrada,
          dataEntradaEstoqueTimestamp: Number.isFinite(dataEntradaTimestamp) ? dataEntradaTimestamp : null,
          valorUnitario: it.valorUnit,
          valorTotal: it.valorTotal,
          formaId,
          formaDescricao,
          itemDespesaId: it.itemInfo ? it.itemInfo.id : null,
          itemDespesaDescricao: it.itemInfo ? it.itemInfo.descricao : null,
          itemDespesaCategoria: it.itemInfo ? it.itemInfo.categoria : null,
          itemDespesaGrupo: it.itemGrupo,
          itemDespesaUnidade: it.itemInfo ? it.itemInfo.unidade : null,
          itemDespesaCodBarras: it.itemInfo ? it.itemInfo.codigoBarras : null,
          gerouEstoqueInsumo: it.deveGerarEstoque,
          estoqueInsumoDocId: it.estoqueInsumoDocId,
          estoqueInsumoDeltaQtd: it.deveGerarEstoque ? it.quantidade : null,
          estoqueInsumoCustoUnitEntrada: it.deveGerarEstoque ? it.valorUnit : null,
          estoqueInsumoPrevUltimaEntradaData: it.deveGerarEstoque ? (estoquePrevMap.get(it.estoqueInsumoDocId)?.ultimaEntradaData || "") : null,
          estoqueInsumoPrevUltimaEntradaDataTimestamp: it.deveGerarEstoque ? (estoquePrevMap.get(it.estoqueInsumoDocId)?.ultimaEntradaDataTimestamp ?? null) : null,
          estoqueInsumoPrevUltimoCustoUnit: it.deveGerarEstoque ? (estoquePrevMap.get(it.estoqueInsumoDocId)?.ultimoCustoUnit ?? null) : null,
          criadoEm: firebase.firestore.FieldValue.serverTimestamp(),
        });
      });
    });
    ...
    if (despMessage) {
      despMessage.textContent = e?.message || "Erro ao salvar despesas.";
      despMessage.className = "msg error";
    }
  }
}
