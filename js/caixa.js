...
const receberTotalPendenteEl = document.getElementById("receber-total-pendente");
const receberTotalPedidosEl = document.getElementById("receber-total-pedidos");
const receberTotalVencidoEl = document.getElementById("receber-total-vencido");
const receberTbody = document.getElementById("receber-tbody");
const receberMessage = document.getElementById("receber-message");
...
function receberShowMsg(texto, tipo) {
  if (!receberMessage) return;
  receberMessage.textContent = texto || "";
  receberMessage.className = "msg";
  if (tipo === "ok") receberMessage.classList.add("ok");
  if (tipo === "error") receberMessage.classList.add("error");
}

function receberNormalizarCondicao(valor) {
  const raw = String(valor || "").trim().toLowerCase();
  if (["prazo", "a prazo", "a_prazo"].includes(raw)) return "prazo";
  return "vista";
}

function receberStatusLabel(item) {
  return item.vencido ? "Vencido" : "Pendente";
}
...
function renderCaixaCharts() {
  ...
  // Estoque (produtos acabados) fechado no Resultado do mês
  // Chave: YYYY-MM
  const estoqueProdutosMap = new Map();
  (resultadosFechadosCache || []).forEach((r) => {
    const k = String(r.mesRef || r.id || "").trim();
    if (!k) return;
    estoqueProdutosMap.set(k, Number(r.estoqueProdutosFinal || 0));
  });
  ...
  const estoquesProdutos = lista.map((r) => {
    const mesRef = String(r.dataIso || "").slice(0, 7); // YYYY-MM
    return estoqueProdutosMap.has(mesRef) ? Number(estoqueProdutosMap.get(mesRef) || 0) : 0;
  });
  ...
  if (chartCaixaSaldo) {
    ...
    // compat: se o gráfico antigo não tiver o 3º dataset, adiciona
    if ((chartCaixaSaldo.data.datasets || []).length < 3) {
      chartCaixaSaldo.data.datasets.push({
        label: "Estoque (produtos) (R$)",
        data: [],
    ...
    chartCaixaSaldo.data.datasets[2].data = estoquesProdutos;
    ...
    const totalLabelPlugin = {
      ...
      afterDatasetsDraw(chart) {
        ...
        for (let i = 0; i < len; i++) {
          ...
          const estProd = Number(chart.data.datasets?.[2]?.data?.[i] || 0);
          const total = banco + est + estProd;
...
// Clientes a receber
// ----------------------
async function carregarClientesAReceber() {
  if (!receberTbody) return;

  receberShowMsg("", "");
  receberTbody.innerHTML = '<tr><td colspan="8">Carregando...</td></tr>';
  ...
  try {
    ...
    snap.forEach((doc) => {
      const d = doc.data() || {};
      const condicao = receberNormalizarCondicao(d.condicaoPagamento || d.condicaoPagamentoLabel);
      const status = String(d.statusRecebimento || "").trim().toLowerCase();

      if (condicao !== "prazo") return;
      if (status === "recebido") return;

      const pedidoChave = String(d.pedidoChave || [
        d.data || "",
        d.clienteId || "",
        d.formaId || "",
        d.condicaoPagamento || "",
      ...
      const atual = grupos.get(pedidoChave) || {
        pedidoChave,
        docIds: [],
        clienteNome: d.clienteNome || "Cliente sem nome",
        formaDescricao: d.formaDescricao || "",
        condicaoPagamentoLabel: d.condicaoPagamentoLabel || "A prazo",
        dataVenda: d.data || "",
      ...
      atual.docIds.push(doc.id);
      atual.valorTotal += Number(d.valorTotal || 0);
      atual.quantidadeItens += Number(d.quantidade || 0);
      if (!atual.dataVenda || (d.data && d.data < atual.dataVenda)) atual.dataVenda = d.data || atual.dataVenda;
      if (!atual.dataPagamento || (d.dataPagamento && d.dataPagamento < atual.dataPagamento)) atual.dataPagamento = d.dataPagamento || atual.dataPagamento;
      if (!atual.formaDescricao && d.formaDescricao) atual.formaDescricao = d.formaDescricao;
      grupos.set(pedidoChave, atual);
    });

    receberPendenciasCache = Array.from(grupos.values()).map((item) => ({
      ...item,
      vencido: !!(item.dataPagamento && item.dataPagamento < hojeIso),
    })).sort((a, b) => {
      if (a.vencido !== b.vencido) return a.vencido ? -1 : 1;
      return String(a.dataPagamento || "9999-99-99").localeCompare(String(b.dataPagamento || "9999-99-99"));
    });

    renderClientesAReceber();
  } catch (e) {
    console.error("Erro ao carregar clientes a receber:", e);
    receberTbody.innerHTML = '<tr><td colspan="8">Erro ao carregar clientes a receber.</td></tr>';
    receberShowMsg("Erro ao carregar clientes a receber.", "error");
  }
}

function renderClientesAReceber() {
  if (!receberTbody) return;

  const lista = receberPendenciasCache || [];
  const totalPendente = lista.reduce((acc, item) => acc + Number(item.valorTotal || 0), 0);
  const totalVencido = lista.reduce((acc, item) => acc + (item.vencido ? Number(item.valorTotal || 0) : 0), 0);

  if (receberTotalPendenteEl) receberTotalPendenteEl.textContent = caixaFormatarMoedaBR(totalPendente);
  if (receberTotalPedidosEl) receberTotalPedidosEl.textContent = String(lista.length);
  if (receberTotalVencidoEl) receberTotalVencidoEl.textContent = caixaFormatarMoedaBR(totalVencido);

  if (!lista.length) {
    receberTbody.innerHTML = '<tr><td colspan="8">Nenhum cliente a receber.</td></tr>';
    return;
  }

  receberTbody.innerHTML = "";

  lista.forEach((item) => {
    const tr = document.createElement("tr");

    const tdVenc = document.createElement("td");
    tdVenc.textContent = item.dataPagamento ? caixaFormatarDataBR(item.dataPagamento) : "-";
    tr.appendChild(tdVenc);

    const tdVenda = document.createElement("td");
    tdVenda.textContent = item.dataVenda ? caixaFormatarDataBR(item.dataVenda) : "-";
    tr.appendChild(tdVenda);

    const tdCliente = document.createElement("td");
    tdCliente.innerHTML = `<strong>${item.clienteNome || "Cliente"}</strong><div class="table-meta">${item.quantidadeItens || 0} un.</div>`;
    tr.appendChild(tdCliente);

    const tdForma = document.createElement("td");
    tdForma.textContent = item.formaDescricao || "-";
    tr.appendChild(tdForma);

    const tdCond = document.createElement("td");
    tdCond.textContent = item.condicaoPagamentoLabel || "A prazo";
    tr.appendChild(tdCond);

    const tdValor = document.createElement("td");
    tdValor.textContent = caixaFormatarMoedaBR(item.valorTotal || 0);
    tr.appendChild(tdValor);

    const tdStatus = document.createElement("td");
    const statusSpan = document.createElement("span");
    statusSpan.className = `status-pill ${item.vencido ? "status-pill--danger" : "status-pill--warning"}`;
    statusSpan.textContent = receberStatusLabel(item);
    tdStatus.appendChild(statusSpan);
    tr.appendChild(tdStatus);

    const tdAcoes = document.createElement("td");
    const btnRecebido = document.createElement("button");
    btnRecebido.type = "button";
    btnRecebido.className = "btn-small btn-success";
    btnRecebido.textContent = "Marcar como recebido";
    btnRecebido.addEventListener("click", () => marcarPedidoComoRecebido(item));
    tdAcoes.appendChild(btnRecebido);
    tr.appendChild(tdAcoes);

    receberTbody.appendChild(tr);
  });
}

async function marcarPedidoComoRecebido(item) {
  if (!item || !Array.isArray(item.docIds) || !item.docIds.length) return;

  const ok = window.confirm(
    `Confirmar recebimento de ${caixaFormatarMoedaBR(item.valorTotal || 0)} do cliente ${item.clienteNome || "selecionado"}?`
  );
  if (!ok) return;

  try {
    const batch = db.batch();
    item.docIds.forEach((docId) => {
    ...
    receberShowMsg(`Recebimento confirmado para ${item.clienteNome || "cliente"}.`, "ok");
    await carregarClientesAReceber();
    await carregarUltimasVendas();
  } catch (e) {
    console.error("Erro ao marcar recebimento:", e);
    receberShowMsg("Erro ao marcar como recebido. Tente novamente.", "error");
  }
}
...
async function calcularResultadoDoMes(mesRef) {
  ...
  const estoqueProdutosFinal = (typeof obterValorEstoqueProdutosAcabadosAtual === "function")
    ? await obterValorEstoqueProdutosAcabadosAtual()
    : 0;
...
async function salvarResultadoDoMes(res) {
  ...
  const payload = {
    ...
    estoqueProdutosFinal: Number(res.estoqueProdutosFinal || 0),
  ...
  lista.forEach((r) => {
    ...
    btnDel.addEventListener("click", async () => {
      ...
      try {
        ...
        await db.collection("custosProducaoMensal").doc(r.mesRef || r.id).delete().catch(() => {});
  ...
  if (!resultadoInitDone) {
    ...
    btnFecharResultado.addEventListener("click", async () => {
      ...
      try {
        ...
        if (typeof gerarSnapshotCustosProducaoMensal === "function") {
          await gerarSnapshotCustosProducaoMensal(mesRef);
        }
