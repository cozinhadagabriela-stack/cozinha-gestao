...
function renderTabelaSaldosInsumos() {
  ...
  if (!estoqueInsumosCache.length) {
    insTbody.innerHTML = '<tr><td colspan="8">Nenhum saldo cadastrado.</td></tr>';
  ...
  if (!lista.length) {
    insTbody.innerHTML = '<tr><td colspan="8">Nenhum item encontrado com os filtros.</td></tr>';
    atualizarGraficoEstoqueInsumos([]);
    return;
  }
...
async function carregarEstoqueInsumosSaldos() {
  ...
  try {
    ...
    if (insTbody) {
      insTbody.innerHTML = '<tr><td colspan="8">Erro ao carregar saldos.</td></tr>';
    }
    estoqueInsumosCache = [];
    atualizarGraficoEstoqueInsumos();
  }
