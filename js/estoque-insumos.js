...
// - Controle por item de despesa (itens)
...
// ----------------------
// Carregar itens (itens) para o select
// ----------------------
async function carregarItensParaEstoqueInsumos() {
  if (!db) return;

  try {
    const snap = await db.collection("itens").orderBy("descricao").get();
...
async function ajustarSaldoEstoqueInsumo(params) {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (qtdNova < 0) {
      throw new Error("Movimento deixaria o estoque de insumos negativo.");
    }
...
function atualizarGraficoEstoqueInsumos(lista) {
  ...
  const truncarRotulo = (texto, limite = isMobile ? 24 : 38) => {
    const t = String(texto || "").trim();
    if (!t) return "";
    return t.length > limite ? `${t.slice(0, Math.max(0, limite - 1)).trimEnd()}…` : t;
  };

  // agrega por insumo
  const porItem = {};
  dados.forEach((r) => {
    const k = (r.itemDescricao || "(Sem nome)").trim();
    const qtd = Number(r.quantidade || 0);
    if (!k || Number.isNaN(qtd) || qtd <= 0) return;
    porItem[k] = (porItem[k] || 0) + qtd;
  });

  const labels = Object.keys(porItem).sort((a, b) => (porItem[b] || 0) - (porItem[a] || 0));
  ...
  const wrapper = insCanvas.parentElement;
  const alturaPorLinha = isMobile ? 30 : 42;
  const alturaBase = isMobile ? 140 : 70;
  const alturaMax = isMobile ? 980 : 680;
  const altura = Math.max(isMobile ? 320 : 260, Math.min(alturaMax, labels.length * alturaPorLinha + alturaBase));
  if (wrapper) wrapper.style.height = `${altura}px`;
  insCanvas.style.height = `${altura}px`;
  insCanvas.style.maxHeight = 'none';
  ...
  chartEstoqueInsumos = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        borderRadius: 8,
        borderSkipped: false,
        categoryPercentage: isMobile ? 0.72 : 0.78,
        barPercentage: isMobile ? 0.74 : 0.82,
      }],
    },
    options: {
      responsive: true,
        ...
        y: {
          ...
          ticks: {
            ...
            callback(value, index) {
              return truncarRotulo(labels[index] || "");
            },
          },
          title: {
            display: !isMobile,
